...
import ExecutionWorkflowContextProvider, {
...
const ExecutionWorkflow = ({ title, showButton, foot, tableHeader, items }) => {
  const useHeaderDropdownActive = useRecoilValue(headerDropdownAtom);
  const { activeTab, setActiveTab } = useContext(executionWorkflowContext);
...
const ExecutionWorkflowWrapper = ({
  title,
  showButton,
  foot,
  tableHeader,
  ...
  return (
    <ExecutionWorkflowContextProvider>
      <ExecutionWorkflow
        title={title}
...
export default ExecutionWorkflowWrapper;
